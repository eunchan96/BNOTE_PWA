    ...
    <div className="flex flex-1 flex-col bg-surface-background">
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {content.intro.map((line, i) => (
          <p key={i} className="mb-1 text-15 leading-relaxed text-text-primary">
            {line}
          </p>
        ))}
        ...
        {content.commandments.map((item) => (
          ...
          >
            <span className="text-brown-primary">{item.text.slice(0, 4)}</span>
            {item.text.slice(4)}
          </p>
        ))}

        <p className="mb-4 text-13 text-text-hint">({content.reference})</p>

        <div className="border-t border-divider" />
        <div className="h-3" />

        <p className="mb-1 text-15 leading-relaxed text-text-primary">
          {content.summary.text}
        </p>
        <p className="text-13 text-text-hint">({content.summary.reference})</p>
      </div>
    </div>
  );
}
