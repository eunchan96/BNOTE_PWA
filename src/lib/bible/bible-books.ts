...
export const BIBLE_BOOKS: BibleBook[] = BOOK_NAMES.map((name, index) => ({
  id: index + 1,
  name,
...
export function chapterUnit(bookId: number): string {
  return bookId === 19 ? "편" : "장";
}

export function nextChapter(bookId: number, chapter: number): { bookId: number; chapter: number } | null {
  ...
  const nextBook = getBook(bookId + 1);
  return nextBook ? { bookId: nextBook.id, chapter: 1 } : null;
}

export function previousChapter(bookId: number, chapter: number): { bookId: number; chapter: number } | null {
  if (chapter > 1) {
    return { bookId, chapter: chapter - 1 };
  }
  const previousBook = getBook(bookId - 1);
  return previousBook ? { bookId: previousBook.id, chapter: previousBook.chapterCount } : null;
}